...
static func play_impact(target_sprite: TextureRect, effect_layer: Control, texture: Texture2D) -> void:
	...
	jolt.tween_property(target_sprite, "position", original_pos + Vector2(4, 0), 0.04)
	jolt.tween_property(target_sprite, "position", original_pos - Vector2(4, 0), 0.05)
	jolt.tween_property(target_sprite, "position", original_pos, 0.04)
...
static func play_projectile(attacker_sprite: TextureRect, target_sprite: TextureRect, effect_layer: Control, texture: Texture2D, on_impact: Callable) -> void:
	...
	var tween := effect.create_tween()
	tween.tween_property(effect, "position", dest, 0.32).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
	tween.parallel().tween_property(effect, "scale", Vector2(0.85, 0.85), 0.32)
...
static func play_status_wave(caster_sprite: TextureRect, effect_layer: Control) -> void:
	...
	tween.set_parallel(true)
	tween.tween_property(ring, "scale", Vector2(2.2, 2.2), 0.42).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
...
static func play_stat_aura(target_sprite: TextureRect, effect_layer: Control, is_buff: bool) -> void:
	...
	var tween := ring.create_tween()
	tween.set_parallel(true)
	tween.tween_property(ring, "scale", Vector2(1.6, 1.6), 0.5).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	tween.tween_property(ring, "position", ring.position + drift, 0.5).set_trans(Tween.TRANS_SINE)
...
static func play_status_condition(target_sprite: TextureRect, effect_layer: Control, status_key: String) -> void:
	...
	for i in range(2):
		flash.tween_property(target_sprite, "modulate", color, 0.09)
		flash.tween_property(target_sprite, "modulate", original_modulate, 0.09)
	...
	tween.tween_property(glyph, "modulate:a", 1.0, 0.12)
	tween.parallel().tween_property(glyph, "position", glyph.position + Vector2(0, -14), 0.55).set_trans(Tween.TRANS_SINE)
	tween.tween_property(glyph, "modulate:a", 0.0, 0.2)
...
static func play_heal_glow(target_sprite: TextureRect, effect_layer: Control) -> void:
	...
	var original_modulate := target_sprite.modulate
	var flash := target_sprite.create_tween()
	flash.tween_property(target_sprite, "modulate", color.lerp(original_modulate, 0.35), 0.12)
	flash.tween_property(target_sprite, "modulate", original_modulate, 0.28)
	...
	for i in range(4):
		...
		tween.tween_interval(randf_range(0.0, 0.12))
		tween.tween_property(mote, "modulate:a", 0.9, 0.1)
		tween.parallel().tween_property(mote, "position", mote.position + Vector2(0, -46), 0.5).set_trans(Tween.TRANS_SINE)
		tween.tween_property(mote, "modulate:a", 0.0, 0.18)
